...
def test_lda_model_initial_values(
    ...
    assert str(hash(dummy_lda_model.createdAt)) == dummy_lda_model.id
    assert str(hash(dummy_lda_model.createdAt)) == dummy_lda_model.getId()
...
def test_lda_model_initial_values_no_creation_paramers(
    ...
    assert (
        str(hash(dummy_lda_model_no_creation_parameters.createdAt))
        == dummy_lda_model_no_creation_parameters.id
    )
    assert (
        str(hash(dummy_lda_model_no_creation_parameters.createdAt))
        == dummy_lda_model_no_creation_parameters.getId()
    )
